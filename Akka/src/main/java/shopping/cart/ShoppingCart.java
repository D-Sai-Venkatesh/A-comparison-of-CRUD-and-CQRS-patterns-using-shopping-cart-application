...
//      checkoutDate = Optional.of(now);
  ...
  public static final class FailCart implements Command {
    final ActorRef<Summary> replyTo;

    @JsonCreator
    public FailCart(ActorRef<Summary> replyTo) {
      this.replyTo = replyTo;
    }
  ...
  private ShoppingCart(String cartId, String projectionTag) {
    ...
    System.out.println("================== Shopping Cart Started " + stringDate + "====================");
  ...
  private ReplyEffect<Event, State> onAdjustItemQuantity(State state, AdjustItemQuantity cmd) {
    System.out.println(cmd.itemId);
    if(cmd.itemId == "fail") {
      Date date = new Date();
      DateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
      String stringDate = sdf.format(date);
      System.out.println("================== Shopping Cart adjust failed " + stringDate + "====================");
      logger.info("====================== Shopping Cart adjust failed ====================");
      int i = 100/0;
    }
    if (cmd.quantity <= 0) {
      logger.info("====================== Shopping Cart adjust failed less than 0 ====================");
      int i = 100/0;
      return Effect().reply(cmd.replyTo, StatusReply.error("Quantity must be greater than zero"));
  ...
  private CommandHandlerWithReplyBuilderByState<Command, Event, State, State> getCommandHandler() {
    return newCommandHandlerWithReplyBuilder()
        ...
        .onCommand(FailCart.class, this::onFailCart);
  }

  private ReplyEffect<Event, State> onFailCart(State state, FailCart cmd) {
